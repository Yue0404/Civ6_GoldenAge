...
function PlantOrBreed(pResourceImprovement, pPlot, iPlayerID)
	...
	if found_transplant_type then
		...
		ResourceBuilder.SetResourceType(pPlot, GameInfo.Resources[transplant_resource_type].Index, 1);	
		ImprovementBuilder.SetImprovementType(pPlot, NONE, NONE);
		--更换地块所属以刷新
		...
		if pPlayer:IsMajor() then
			...
			if pCity then
				...
				local bAutoPurchaseSuccess = ExposedMembers.ResourceCultivation.GA_CultivationBuyPlot(iCityID, iPlotIndex, iPlayerID);
			else
				print('出错了！没找到原城市');
			end
		end
		if not bAutoPurchaseSuccess then
