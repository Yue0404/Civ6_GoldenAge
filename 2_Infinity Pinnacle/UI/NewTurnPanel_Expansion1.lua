	...
	if (eraCountdown ~= nil and eraCountdown ~= 0 and eraCountdown ~= m_LastShownCountdownTurn and currentEra ~= nil) then
		...
		if (currentEraInfo ~= nil) then
			...
			if currentEra == 4 then
				Controls.NewTurnLabel:SetText(Locale.ToUpper(Locale.Lookup("LOC_NEW_TURN_PANEL_NEXT_ERA_COUNTDOWN_WORLDWAR_"..eraCountdown, eraCountdown, currentEraInfo.Name)));
			else
