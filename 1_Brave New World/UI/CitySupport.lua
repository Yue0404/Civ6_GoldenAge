...
--  GoldenAge (Special thanks for Hemmelfort's technical support)
-- ===========================================================================
...
-- 辉煌与至暗时代
-- =========================
local iFutureTech = GameInfo.Technologies['TECH_FUTURE_TECH'].Index;
local iFutureCivic = GameInfo.Civics["CIVIC_FUTURE_CIVIC"].Index;

local function GA_FrontTechEra(iPlayer)
    local pTechs = Players[iPlayer]:GetTechs()
    local iCurrentEra = 0
    for tech in GameInfo.Technologies() do
        if pTechs:HasTech(tech.Index) then
            local era = GameInfo.Eras[tech.EraType]
            if (era ~= nil) and (era.Index > iCurrentEra) then
                iCurrentEra = era.Index
            end
        end
    end
    return iCurrentEra
...
local function GA_FrontCivicEra(iPlayer)
    local pCivic = Players[iPlayer]:GetCulture()
    local iCurrentEra = 0
    for civic in GameInfo.Civics() do
        if pCivic:HasCivic(civic.Index) then
...
local function GA_GetFrontEras(iPlayer)
    local turn = Game.GetCurrentGameTurn()
    local c = g_EraCache[iPlayer]
    if c and c.turn == turn then
        return c.techEra, c.civicEra
    end
    local techEra = GA_FrontTechEra(iPlayer)
    local civicEra = GA_FrontCivicEra(iPlayer)
    g_EraCache[iPlayer] = { turn = turn, techEra = techEra, civicEra = civicEra }
    return techEra, civicEra
end

function HistoricalEra(pCity)
    local iPlayer = pCity:GetOwner()   
	local pPlayer = Players[iPlayer];
    local sCivType = PlayerConfigurations[iPlayer]:GetCivilizationTypeName()
	if not(GameInfo.GA_RiseFall) then return "" end
    local tab = GameInfo.GA_RiseFall[sCivType]
    local sResult2 = '0%'
    if (tab ~= nil) then
        local iTechEra, iCivicEra = GA_GetFrontEras(iPlayer)
		local iCurrentEra = math.min(iTechEra, iCivicEra)
        local sCurrentEra = GameInfo.Eras[iCurrentEra].EraType
		local bHasFutureTech = pPlayer:GetTechs():HasTech(iFutureTech);
		local bHasFutureCivic = pPlayer:GetCulture():HasCivic(iFutureCivic);
		if (bHasFutureTech and bHasFutureCivic) then
            sResult2 = '0%'
		elseif (sCurrentEra == tab.GloriousEra) then
            sResult2 = '+15%'
        elseif (sCurrentEra == tab.DarkEra) then
            sResult2 = '-15%'
        end
    end
...
-- 根据人口 + 市政厅等级计算惩罚
local function CalcDebuff(iPopulation, iCityHallLevel)
    local iBase = 0
...
local function HasGovWideBuilding(iPlayerID)
    ...
    if not pPlayer or not GOV_WIDE_BUILDING then
        return false
    end
...
-- 主函数
function CrazyExpansion(pCity)
    if not pCity then return "" end
    local iPlayerID = pCity:GetOwner()
    local pPlayer = Players[iPlayerID]
    if not pPlayer then return "" end
    local iCityCount = pPlayer:GetCities():GetCount()
    local iPopulation = pCity:GetPopulation()
    -- 市政厅等级
    local iCityHallLevel = GetCityHallLevel(pCity)

    -- 基础惩罚
    local iDebuffRate = CalcDebuff(iPopulation, iCityHallLevel)

    -- 祠堂：全国爆铺惩罚 -1（仅一次）
    if iDebuffRate > 0 and HasGovWideBuilding(iPlayerID) then
        iDebuffRate = iDebuffRate - 1
    end

    if iDebuffRate <= 0 then
        return ""
    end

    local iTotalPenalty = (iCityCount - 1) * iDebuffRate
    if iTotalPenalty <= 0 then
        return ""
    end
...
-- 城市内政
-- =========================
function CityPolicy(pCity, pYieldType)
    if not pCity or not pYieldType then return "" end
	if not(GameInfo.Buildings["BUILDING_CITIPOLICY_NONE"]) then return "" end
	
    local pNoCityPolicy = GameInfo.Buildings["BUILDING_CITIPOLICY_NONE"]

    local iIndexToFind = "BUILDING_CITIPOLICY_YIELD_" .. tostring(pYieldType)
    local pCityPolicy = GameInfo.Buildings[iIndexToFind]
    if pCityPolicy == nil then return "" end

    local pBuildings = pCity:GetBuildings()
    local iYieldRate = "0%"

    if pBuildings:HasBuilding(pCityPolicy.Index) then
        iYieldRate = "+20%"
    elseif pBuildings:HasBuilding(pNoCityPolicy.Index) then
        iYieldRate = "0%"
    else
        iYieldRate = "-5%"
    end
...
function UpdateYieldData( pCity:table, data:table )
	...
	data.CulturePerTurnToolTip		= pCity:GetYieldToolTip(YieldTypes.CULTURE) .. CrazyExpansion(pCity).. HistoricalEra(pCity)..CityPolicy(pCity, "CULTURE");

	data.FaithPerTurn				= Round( pCity:GetYield( YieldTypes.FAITH ), 1);
	data.FaithPerTurnToolTip		= pCity:GetYieldToolTip(YieldTypes.FAITH) .. CrazyExpansion(pCity).. HistoricalEra(pCity)..CityPolicy(pCity, "FAITH");

	data.FoodPerTurn				= Round( pCity:GetYield( YieldTypes.FOOD ), 1);
	data.FoodPerTurnToolTip			= pCity:GetYieldToolTip(YieldTypes.FOOD) .. HistoricalEra(pCity)..CityPolicy(pCity, "FOOD");

	data.GoldPerTurn				= Round( pCity:GetYield( YieldTypes.GOLD ), 1);
	data.GoldPerTurnToolTip			= pCity:GetYieldToolTip(YieldTypes.GOLD) .. CrazyExpansion(pCity).. HistoricalEra(pCity)..CityPolicy(pCity, "GOLD");

	data.ProductionPerTurn			= Round( pCity:GetYield( YieldTypes.PRODUCTION ),1);
	data.ProductionPerTurnToolTip	= pCity:GetYieldToolTip(YieldTypes.PRODUCTION) .. HistoricalEra(pCity)..CityPolicy(pCity, "PRODUCTION");

	data.SciencePerTurn				= Round( pCity:GetYield( YieldTypes.SCIENCE ), 1);
	data.SciencePerTurnToolTip		= pCity:GetYieldToolTip(YieldTypes.SCIENCE) .. CrazyExpansion(pCity).. HistoricalEra(pCity)..CityPolicy(pCity, "SCIENCE");
